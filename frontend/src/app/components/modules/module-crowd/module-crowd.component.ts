...
@Component({
  selector: 'app-module-crowd',
  templateUrl: './module-crowd.component.html',
  styleUrls: ['./module-crowd.component.css']
})
export class ModuleCrowdComponent implements OnInit, OnDestroy {
  ...
  enabledFeatures: EnabledFeatures = {
    human: false, vehicle: false, helmet: false, loitering: false, crowd: true,
  ...
  private updateHistory(result: DetectionResult): void {
    const density = result.crowd_density || 0;
    const occupied = result.occupied_area || 0;
    const label = `${density} density, ${occupied}% occupied`;
    this.history.unshift({ timestamp: new Date(), value: density, label });
