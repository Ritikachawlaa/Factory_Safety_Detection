...
@Component({
  selector: 'app-module-helmet',
  templateUrl: './module-helmet.component.html',
  styleUrls: ['./module-helmet.component.css']
})
export class ModuleHelmetComponent implements OnInit, OnDestroy {
  ...
  private updateHistory(result: DetectionResult): void {
    const value = result.ppe_compliance_rate;
    const label = `${result.ppe_compliance_rate}% compliant, ${result.helmet_violations} violations`;
  ...
  private updateStats(result: DetectionResult): void {
    if (result.ppe_compliance_rate > this.peakCount) {
      this.peakCount = result.ppe_compliance_rate;
    }
