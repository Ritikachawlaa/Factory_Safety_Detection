...
@Component({
  selector: 'app-module-loitering',
  templateUrl: './module-loitering.component.html',
  styleUrls: ['./module-loitering.component.css']
})
export class ModuleLoiteringComponent implements OnInit, OnDestroy {
  ...
  enabledFeatures: EnabledFeatures = {
    human: false, vehicle: false, helmet: false, loitering: true, crowd: false,
  ...
  private updateHistory(result: DetectionResult): void {
    const value = result.loitering_count;
    const label = result.loitering_detected ? `⚠️ Loitering detected (${result.loitering_count})` : 'Area clear';
  ...
  private updateStats(result: DetectionResult): void {
    if (result.loitering_count > this.peakCount) this.peakCount = result.loitering_count;
