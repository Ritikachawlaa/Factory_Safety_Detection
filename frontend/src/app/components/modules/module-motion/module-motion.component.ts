...
@Component({
  selector: 'app-module-motion',
  templateUrl: './module-motion.component.html',
  styleUrls: ['./module-motion.component.css']
})
export class ModuleMotionComponent implements OnInit, OnDestroy {
  ...
  enabledFeatures: EnabledFeatures = {
    ...
    box_count: false, line_crossing: false, tracking: false, motion: true,
  ...
  private updateHistory(result: DetectionResult): void {
    const detected = result.motion_detected;
    const intensity = result.motion_intensity || 0;
    const label = detected ? `💨 Motion detected: ${intensity}%` : 'No motion';
    this.history.unshift({ timestamp: new Date(), value: intensity, label });
