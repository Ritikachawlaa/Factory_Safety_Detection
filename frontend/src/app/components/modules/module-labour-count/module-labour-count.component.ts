...
@Component({
  selector: 'app-module-labour-count',
  templateUrl: './module-labour-count.component.html',
  styleUrls: ['./module-labour-count.component.css']
})
export class ModuleLabourCountComponent implements OnInit, OnDestroy {
  ...
  private updateHistory(result: DetectionResult): void {
    const value = result.labour_count;
    const label = `${result.labour_count} workers present`;
  ...
  private updateStats(result: DetectionResult): void {
    if (result.labour_count > this.peakCount) this.peakCount = result.labour_count;
